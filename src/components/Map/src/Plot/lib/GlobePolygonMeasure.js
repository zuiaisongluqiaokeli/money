...
GlobePolygonMeasure.prototype = {
  ...
  okHandler: null,
  cancelHandler: null,
  dragIcon: circle_gray_img,
  dragIconLight: circle_red_img,
  material: null,
  ...
  _getMeasureTip: function(pntList) {
    //计算周长时，需要首尾相连
    var temPntList = [...pntList, pntList[0]];

    var _this = this;
    var dis2d = _this._computeLineDis2d(temPntList);
    var dis3d = _this._computeLineDis3d(temPntList);
    dis2d = dis2d.toFixed(3);
    dis3d = dis3d.toFixed(3);
    var tip = "周长：" + dis3d + "千米";
    if (temPntList.length > 2) {
      var area = _this._computeArea(temPntList);
  ...
  _createToolBar: function() {
    ...
    objs.remove();
    var html =
      '<div id="shapeEditContainer" style="padding: 10px 10px;">' +
    ...
    btnOK.unbind("click").bind("click", function() {
      if (_this.okHandler) {
        ...
        //计算周长时，需要首尾相连
        var tempPositions = [...positions, positions[0]];

        var dis2d = _this._computeLineDis2d(tempPositions);
        var dis3d = _this._computeLineDis3d(tempPositions);
        var area = _this._computeArea(tempPositions);
        dis2d = dis2d.toFixed(3);
        dis3d = dis3d.toFixed(3);
        area = area.toFixed(3);
