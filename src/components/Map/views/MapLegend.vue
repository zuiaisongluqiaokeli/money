<template>
    <div class="map-legend" v-if="allEntityBackEnd.length > 0">
        <div class="remove-all">
            ...
            <i :class="collapse ? 'el-icon-top-right' : 'el-icon-bottom-left'" @click="collapse = !collapse"></i>
        </div>
        <div v-show="!collapse">
            <div class="title">
            ...
            <div class="legend">
                <el-collapse v-for="(item, index) in tableData" :key="index">
                    <el-collapse-item>
                        <template slot="title">
                            <div class="titleItem" style="padding-left: 31px; font-size: 15px">
                                ...
                                <el-tooltip class="item" effect="dark" :content="`${item.category}(${item.number})`" placement="top-start">
                            ...
                            <div class="switch" style="padding-right: 8px">
                                <i class="el-icon-view" title="切换显示" :class="{
                    'is-active': item.visible,
                  }" @click="active == 1 ?changeVisibleEntities($event,index, item):eventPrevent($event)"></i>
                                <i class="el-icon-delete" title="移除" @click="removeEntities($event,item, index)"></i>
                                <i class="el-icon-aim" title="切换攻击范围" :class="{
                    'is-active': item.attackRange,
                  }" @click="active == 1 ?changeEntitiesAttackRange($event,index, item):eventPrevent($event)"></i>
                            </div>
                        </template>
                        <div v-for="(ele, key) in item.listInfo" :key="key">
                            <div class="titleItem" @click="ele.properties.latitude ? flyEntity(ele) : mapMark(ele)" :title="ele.properties.latitude ? '飞向该点' : '暂无坐标'">
                                <!-- <img :src="ele.image" class="img" /> -->

                                <el-tooltip class="item" effect="dark" :content="ele.properties.name|| ele.properties.名称" placement="top-start">
                                    <div class="description">
                  ...
                  }" @click="active == 1?changeVisibleEntity($event,index, ele, key):''"></i>
                                <i class="el-icon-delete" title="移除" @click="active == 1?removeEntity($event,index, ele, key):''"></i>
                                <i class="el-icon-aim" title="切换攻击范围" :class="{
                    'is-active': ele.attackRange,
                  }" @click="active == 1?changeAttackRange($event,index, ele, key):''"></i>
                            </div>
                        </div>
                    </el-collapse-item>
                </el-collapse>
...
export default {
    ...
    methods: {
        ...
        handleLegendDataChange(data) {
            ...
            this.active ==1 ?this.tableData = this.arr1: this.tableData = this.arr2
            this.$set(this.tableData, 0, this.tableData[0]);
