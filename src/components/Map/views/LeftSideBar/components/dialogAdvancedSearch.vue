<template>
    <div class="search-modal" id="search-dialog">
        <el-dialog title="高级搜索" class="dialog" :visible="true" width="1085px" :close-on-click-modal="false" :before-close="emitClosedEvent">
            <div class="dialog-body">
                <el-row class="row" v-loading="verticesListLoading" :element-loading-text="loadingText">
                    <el-col style="overflow: hidden" :span="7">
                        <div class="tree-filter">
                            <el-input v-model="filterText" class="search" placeholder="请输入内容" prefix-icon="el-icon-search"></el-input>
                        </div>
                        <el-tree ref="tree" class="filter-tree" :data="treeData" :props="defaultProps" :filter-node-method="filterNode" highlight-current node-key="id" :expand-on-click-node="false" :default-expanded-keys="defaultExpandedKeys" @node-click="queryNode">
                        </el-tree>
                    </el-col>
                    <el-col :span="16" class="main-right-box" style="overflow: hidden" :offset="1">
                        <div class="panel" style="height: 100%">
                            <div class="head clearfix">
                                <div style="display: inline-block">
                                    <el-form :inline="true" @submit.native.prevent>
                                        <el-form-item>
                                            <el-input v-model="advancedSearchQuery.searchContent" clearable class="search-input" placeholder="请输入实体名称" :disabled="showAdvancedSearch" @keyup.enter.native="findVertices" @clear="findVertices">
                            ...
                            <el-collapse-transition>
                                <div v-show="showAdvancedSearch">
                                    <AdvanceSearch v-if="reFresh" ref="advanceSearch" :advanced-search-query="advancedSearchQuery" :search-type="searchType" :search-relation="searchRelation" :search-key="searchKey" :search-name="searchName" :placeholder="placeholder"></AdvanceSearch>
                            ...
                            <el-table :data="verticesListData" highlight-current-row ref="table" style="width: 100%" @selection-change="handleSelectChange" :header-cell-style="{ 'text-align': 'left' }" :cell-style="{ 'text-align': 'left' }" row-key="id">
                                <el-table-column type="selection" width="30" :reserve-selection="true"></el-table-column>
                                <el-table-column label="实体名称" width="150" show-overflow-tooltip><template slot-scope="scope">
                                        <span class="btn-color" @click="lookDetail(scope.row)" style="cursor: pointer">{{ scope.row.name }}</span>
                                    </template></el-table-column>
                                <el-table-column prop="fullCategoryName" label="实体分类" width="150" show-overflow-tooltip></el-table-column>
                                <el-table-column prop="longitude" label="经度" width="150" show-overflow-tooltip></el-table-column>
                                <el-table-column prop="latitude" label="纬度" show-overflow-tooltip></el-table-column>
                            </el-table>
                            <el-pagination class="pagination" background @size-change="changeSize" @current-change="changeCurrent" :current-page.sync="listQuery1.page" :page-size="listQuery1.size" layout="total, sizes, prev, pager, next, jumper" :total="listQuery1.total"></el-pagination>
                        </div>
                    </el-col>
                </el-row>
            </div>
            ...
            <div class="GisInfoPanel" :class="[drawerDialog ? 'show-panel' : 'hide-panel']" v-if="drawerDialog" append-to-body>
          ...
          <div class="tab-button" @click="drawerDialog=!drawerDialog">
                ...
                <div class="panel-content">
                    ...
                    <div class="body">
                            ...
                            <el-collapse :value="['1', '2', '3', '4']">
                                <el-collapse-item name="1" title="实体信息">
                                    <div class="collapse-body avatar-name">
                                        <div class="avatar">
                                            <img :src="newVerticesData.avatar" v-if="newVerticesData.avatar" :onerror="errorImg" />
                        ...
                        categoryName || "暂未分类"
                                        ...
                                        <div v-for="(item, index) in newVerticesData.propertiesJson" :key="index" class="properties">
                                            <template v-if="
                          item.name !== '实体分类' &&
                          item.name !== '名称' &&
                          item.name !== 'avatar' &&
                    ...
                    查看详细
                                ...
                                <el-collapse-item name="3" title="标签">
                                    <div class="labels collapse-body">
                                        <div v-if="newVerticesData.labelsList.length > 0" style="width: 96%; font-size: 14px">
                                            <el-tag style="margin: 10px 5px" size="mini" v-for="val in newVerticesData.labelsList" :key="val">{{ val }}
                                            </el-tag>
                                        </div>
                                        <div v-else>暂无标签</div>
                                    </div>
                                ...
                                <el-collapse-item name="4" title="相关文档">
                                    <div class="labels collapse-body">
                                        <div v-if="fileData.length > 0" style="width: 96%; font-size: 14px">
                                            <div v-for="(item, index) in fileData" :key="index" class="list-item" :title="item.name">
                                                <div>
                                                    <span :class="item.icon" class="file-icon"></span><span>{{ item.name }}</span>
                                                </div>
            ...
            <el-dialog title="预览文件" :visible.sync="previewDialog" width="60%" append-to-body :before-close="handleClosePreviewDialog">
                ...
                <audio v-if="previewMusic" :src="previewMusic" style="width: 100%; height: 100%; min-height: 100px" controls autoplay></audio>
            </el-dialog>
        </el-dialog>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        async queryNode(node) {
            ...
            this.node = node;
            this.searchCategory = node.parentName === "" ? node.name : node.parentName + ">" + node.name;
            this.nodeClick(node);
            this.advanceQuery();
        },
        nodeClick(data) {
                ...
                if (this.node) {
                    params.moreCondition.forEach((item) => {
                        item.moreConditions.push({
                            ...
                            conditionsEnum: "LEFT",
                            key: "实体分类",
                            value: this.searchCategory.includes("undefined") ? "" : this.searchCategory,
                            valueType: "String",
                            operator: "且",
                        });
                    });
