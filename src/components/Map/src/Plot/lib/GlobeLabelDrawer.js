...
var GlobeLabelDrawer = function() {
  this.init.apply(this, arguments);
};

GlobeLabelDrawer.prototype = {
  viewer: null,
  scene: null,
  clock: null,
  canvas: null,
  ...
  showModifyPoint: function(position, label, okHandler, cancelHandler) {
    var _this = this;
  ...
  startDrawPoint: function(okHandler, cancelHandler) {
    ...
    _this.drawHandler.setInputAction(function(event) {
      ...
      _this.tooltip.showAt(
        wp,
        '<input type="text" id="labelDrawerInput" autocomplete="off" />'
      );
      _this._startModify();
  ...
  _startModify: function() {
    ...
    _this.modifyHandler.setInputAction(function(event) {
      ...
      if (isMoving) {
        ...
        _this.tooltip.showAt(
          wp,
          '<input type="text" id="labelDrawerInput" value="' +
            _this.label +
            '"  autocomplete="off" />'
        );
      } else {
  ...
  _createToolBar: function() {
    ...
    var html =
      ...
      '    <div id="shapeEditRTCorner" style="width: 16px; position: absolute; right: 0px; top: 0px; bottom: 0px"></div>' +
      "</div>";
    $("body").append(html);
    ...
    btnOK.unbind("click").bind("click", function() {
      ...
      layer.close(_this.toolBarIndex);
      if (_this.okHandler) {
        let label = $("#labelDrawerInput").val();
        _this.okHandler(_this.position, label);
      }
    });
