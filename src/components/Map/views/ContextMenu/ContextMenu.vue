...
export default {
    ...
    data() {
        return {
            ...
            dataInnerList: [
                ...
                { name: '搜索', icon: '\ue8ef', angle: 1, action: '' },
                { name: '范围', icon: '\ue814', angle: 1, action: 'scope' },
    ...
    methods: {
        handleClick(data) {
            gisvis.emitter.emit(EventType.CONTEXT_MENU_CLICK, data);
        },
        /**
        ...
        initMenu() {
            ...
            const svg = d3.select('.context-menu .menu-container').append('svg').attr('width', width).attr('height', height);
            ...
            g.append('text')
                .attr('transform', (d) => {
                    return 'translate(' + arc.centroid(d).map((v, i) => (i ? v + 10 : v)) + ')';
                })
                .attr('text-anchor', 'middle')
                .attr('fill', '#1C2833')
            ...
            g.append('text')
                .attr('transform', (d) => {
                    return 'translate(' + arc.centroid(d).map((v, i) => (i ? v - 5 : v)) + ')';
                })
                .attr('text-anchor', 'middle')
                .attr('fill', '#1C2833')
            ...
            gBigOuter
                .append('text')
                .attr('transform', (d) => {
                    return 'translate(' + arcBigOuter.centroid(d).map((v, i) => (i ? v + 5 : v)) + ')';
                })
                .attr('text-anchor', 'middle')
                .attr('fill', '#1C2833')
                .attr('style', 'font-size: 11px')
        ...
        menuInnerHover(index) {
            console.log(index);
            let array;
            document.querySelector('.hover') ? document.querySelector('.hover').classList.remove('hover') : '';
        ...
        menuOuterHover(index) {
            ...
            // let bigArray = ['实体', '文本']
            document.querySelector('.outer.hover') ? document.querySelector('.outer.hover').classList.remove('hover') : '';
            document.querySelectorAll('.big-outer').forEach((v, i) => {
                v.setAttribute('class', 'unshow big-outer');
            });
