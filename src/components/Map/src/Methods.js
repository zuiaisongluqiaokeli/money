...
function calculateCircleOutlineGeometry(options = {}) {
    ...
    const geometry = Cesium.CircleOutlineGeometry.createGeometry(circleOutlineGeometry);
    ...
    const positions = Array.from({ length: length }, (value, index) => {
        ...
        return [float64ArrayPositions[x], float64ArrayPositions[y], float64ArrayPositions[z]];
...
function computeEllipsePositions(options = {}) {
    ...
    const positions = Cesium.Cartesian3.unpackArray(ellipsePositions.outerPositions);
    const [first] = positions;
...
const toString = new Proxy(Object.prototype.toString, {
    apply(target, ctx, arg) {
        ...
        if (arg.length > 1) {
            return Reflect.apply(target, call, arg) === Reflect.apply(target, type, arg);
        }
...
function processGroupData(groupData = []) {
    return groupData.map(item => {
        const { id, name, lng, lat, groupCategory, groupType = groupCategory, radius = 0, country } = transferPropsKey(item);
